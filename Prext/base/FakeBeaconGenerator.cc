...
map<double, FakeInfo> FakeBeaconGenerator::storeReply(VehicleReply* vrply) {
...
//    }
    ...
    if (it == vehreply.end()){
        ...
        replyFake = handleReply(vrply);//call only once for duplicate replies
        return replyFake;
    }
    else return replyFake;
}

map<double, FakeInfo> FakeBeaconGenerator::handleReply(VehicleReply* vrply) {
...
//parse xml file and store essential information
    ...
    char checkf[50];
      sprintf(checkf, "checkfakeinfo.txt");
                    ...
                    outfile.setf(ios::fixed, ios::floatfield);
                    outfile.precision(4);
                    map<double, FakeInfo>::iterator itf = allFakeInfo.begin();
                    ...
                    outfile.close();
    allFakeInfo.erase(allFakeInfo.begin());
    return allFakeInfo;
}
...
map<double, FakeInfo> FakeBeaconGenerator::readXML(int fCount){
        ...
        while(pTime!=0){
                if(pTime->FirstChildElement()!=NULL){
                    ...
                    pVehicle = pTime->FirstChildElement();
                                    fakeI.timestamp = stod(pTime->Attribute("time")) + fRouteCount*0.01;
                                    fakeI.positionx = stod(pVehicle->Attribute("x"));
                                    fakeI.positiony = stod(pVehicle->Attribute("y"));
                                    fakeI.angle = stod(pVehicle->Attribute("angle"));
                                    fakeI.speed = stod(pVehicle->Attribute("speed"));
